...
void DrawGraphs::Update()
{
	...
	scissorRect.right = scissorRect.left + wp->GetWinWidth(); // 切り抜き座標右
	scissorRect.top = 0; // 切り抜き座標上
	scissorRect.bottom = scissorRect.top + wp->GetWinHeight(); // 切り抜き座標下
	...
	// プリミティブ形状の設定コマンド
	wp->cmdList_->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST); // 三角形リスト
	// 頂点バッファビューの設定コマンド
	wp->cmdList_->IASetVertexBuffers(0, 1, &vbView);
	// 描画コマンド
	wp->cmdList_->DrawInstanced((sizeof vertices / sizeof XMFLOAT3), 1, 0, 0); // 全ての頂点を使って描画
